...
function animalPosition(s,i,frame,weather){const need=s.needs||{},goal=String(s.goal||'');const phase=hash01(`${s.id}:phase:${i}`)*Math.PI*2;const area=habitat(s.habitat,hash01(s.id||s.name||'species'));const predator=s.kind==='predator';const runner=s.id==='reed-runner';const cycle=(frame+Math.floor(hash01(`${s.id}:feed:${i}`)*24))%24;const feeding=!predator&&cycle>=18;const mt=motionTick(frame,phase,feeding);const weatherSlow=1-clamp(Number(weather?.precipitationRate||0)*.45+Number(weather?.wind||0)*.12,0,.55);const hunted=Boolean(s.lastHunt&&Number(s.lastHunt.year)>=(Number(s.bornYear||0)));const needBoost=goal.includes('hunt')?1.3:goal.includes('water')||goal.includes('predator')?1.22:goal.includes('rest')?.72:hunted?.92:1;const speed=(predator?.038:runner?.05:.034)*weatherSlow*needBoost;const step=mt*speed;let x=clamp(area.x+Math.sin(step+phase)*area.rx*.62,-32,32),z=clamp(area.z+Math.cos(step*.77+phase*1.4)*area.rz*.62,-32,32);if(goal.includes('water'))x=x+( -18-x)*.18;return{phase,area,feeding:feeding&&Number(need.hunger||0)<.7,x,z};}
