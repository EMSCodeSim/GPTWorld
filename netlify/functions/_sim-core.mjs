...
// Stable per-animal memory: deterministic across requests and shared by both world renderers.
...
export function ecologyRenderEntities(ecosystem,now=Date.now(),weather=null,observer=null,forestPressure=null){
 ...
 for(const s of species.filter(item=>item.kind==='herbivore'&&Number(item.population||0)>0)){const pop=Number(s.population||0),count=Math.max(1,Math.min(9,Math.round(pop/65)));
  for(let i=0;i<count;i++){const base=animalPosition(s,i,frame,weather,forestPressure),nextBase=animalPosition(s,i,frame+1,weather,forestPressure),memory=animalMemory(s,i,frame),herd=herdOffset(s,i,frame),epoch=Math.floor((frame+Math.floor(hash01(`${s.id}:${i}:meal`)*70))/70),local=(frame+Math.floor(hash01(`${s.id}:${i}:meal`)*70))%70,target=plantFoods.length?plantFoods[Math.floor(hash01(`${s.id}:${i}:${epoch}:food`)*plantFoods.length)%plantFoods.length]:null;let x=base.x,z=base.z,nx=nextBase.x,nz=nextBase.z,behavior=base.forestShift>=.2?'migrating':'herding';if(i%3){const leader=animalPosition(s,herd.leader,frame,weather,forestPressure);x=x*.22+(leader.x+herd.dx)*.78;z=z*.22+(leader.z+herd.dz)*.78;nx=x; nz=z;}if(Number(s.needs?.thirst||0)>.65){const p=animalLandRoute({x,z},memory.water,.36,`${s.id}:${i}:water`);x=p.x;z=p.z;behavior='seeking water';}
   if(target&&local>=18&&Number(s.needs?.thirst||0)<=.65){const q=clamp((local-18)/24,0,1),smooth=q*q*(3-2*q),position=animalLandRoute(base,target,smooth,`${s.id}:${i}:${epoch}:food`),next=animalLandRoute(base,target,clamp(smooth+.04,0,1),`${s.id}:${i}:${epoch}:food-next`);x=position.x;z=position.z;nx=next.x;nz=next.z;behavior=local>=42?'feeding':'seeking food';}
   ...
   const safe=outsideTown(x,z,`${s.id}:${i}:${frame}:feeding`,11.5),land=animalLandPoint(safe.x,safe.z,`${s.id}:${i}:${frame}:feeding`),nextLand=animalLandPoint(nx,nz,`${s.id}:${i}:${frame}:feeding-next`);x=land.x;z=land.z;nx=nextLand.x;nz=nextLand.z;const record={s,i,x,z,nx,nz,behavior,target,memory,herdId:`${s.id}-${herd.leader}`,consumedPlant:target&&local>=58?target.id:null,id:`${s.id}-${i}`};herbivoreRecords.push(record);animals.push(record);}
 }
 ...
 for(const s of species.filter(item=>item.kind==='predator'&&Number(item.population||0)>0)){const pop=Number(s.population||0),count=Math.max(1,Math.min(5,Math.round(pop/14)));
  for(let i=0;i<count;i++){const base=animalPosition(s,i,frame,weather,forestPressure),nextBase=animalPosition(s,i,frame+1,weather,forestPressure),offset=Math.floor(hash01(`${s.id}:${i}:hunt`)*110),epoch=Math.floor((frame+offset)/110),local=(frame+offset)%110,prey=herbivoreRecords.length?herbivoreRecords[Math.floor(hash01(`${s.id}:${i}:${epoch}:prey`)*herbivoreRecords.length)%herbivoreRecords.length]:null;let x=base.x,z=base.z,nx=nextBase.x,nz=nextBase.z,behavior='patrolling',successfulHunt=false;const stamina=Number(s.needs?.energy??.85),preyStamina=Number(prey?.s.needs?.energy??.85),huntSuccess=hash01(`${s.id}:${i}:${epoch}:outcome`)<clamp(.43+(stamina-preyStamina)*.3, .12,.8);
   if(prey&&local>=38){const q=clamp((local-38)/52,0,1),smooth=q*q*(3-2*q),position=animalLandRoute(base,prey,smooth,`${s.id}:${i}:${epoch}:hunt`),next=animalLandRoute(base,prey,clamp(smooth+.04,0,1),`${s.id}:${i}:${epoch}:hunt-next`);x=position.x;z=position.z;nx=next.x;nz=next.z;behavior=local<64?'stalking':local<90?'chasing':huntSuccess?'feeding':'recovering';if(local>=90&&!huntSuccess){const retreat=animalLandRoute({x,z},base,.5,`${s.id}:${i}:retreat`);x=retreat.x;z=retreat.z;}if(local>=98&&huntSuccess){killedPrey.add(prey.id);successfulHunt=true;}}
   {const safe=outsideTown(x,z,`${s.id}:${i}:${frame}:hunting`,11.5),land=animalLandPoint(safe.x,safe.z,`${s.id}:${i}:${frame}:hunting`),nextLand=animalLandPoint(nx,nz,`${s.id}:${i}:${frame}:hunting-next`);x=land.x;z=land.z;nx=nextLand.x;nz=nextLand.z;}animals.push({s,i,x,z,nx,nz,behavior,target:prey?.id||null,memory:animalMemory(s,i,frame),successfulHunt});}
 }
 ...
 for(const record of animals){if(record.s.kind==='herbivore'&&killedPrey.has(record.id))continue;const heading=Math.atan2(record.nz-record.z,record.nx-record.x),body=.65+clamp(Number(record.s.traits?.size||.4),0,1)*.8,before=out.length;animalParts(out,record.s,record.i,record.x,record.z,body,heading,record.behavior);const entity=out[before];if(entity){entity.foodWeb=true;entity.foodTarget=record.target?.id||record.target||null;entity.consumedPlant=record.consumedPlant||null;entity.preyKilled=record.s.kind==='predator'&&record.behavior==='feeding';entity.speciesName=record.s.name||record.s.id;entity.needs={hunger:Number(record.s.needs?.hunger??.25),thirst:Number(record.s.needs?.thirst??.2),energy:Number(record.s.needs?.energy??.85),fear:Number(record.s.needs?.fear??0)};entity.health=Number(record.s.health??1);entity.goal=record.s.goal||record.behavior;entity.population=Number(record.s.population||0);entity.memory=record.memory||null;entity.herdId=record.herdId||null;entity.huntSucceeded=Boolean(record.successfulHunt);}}
