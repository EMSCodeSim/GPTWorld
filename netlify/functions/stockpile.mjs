...
export default async req=>{
  ...
  try{
    ...
    if(req.method==='GET'){
      const stockRows=await sql`SELECT value FROM world_state WHERE key='settlement_stockpile' LIMIT 1`;
      const invRows=playerId?await sql`SELECT wood,stone,herbs FROM player_inventory WHERE player_id=${playerId} LIMIT 1`:[];
  ...
  }catch(err){console.error('GPTWorld stockpile error',err);return reply({ok:false,error:'stockpile_failed'},500)}
};
