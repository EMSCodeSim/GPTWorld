...
export function canEnterBuilding(level){
...
export function interiorEntryUrl(buildingId,base='./interior.html'){
  const id=String(buildingId||'');
  if(!isKnownBuilding(id))return null;
  return `${base}?building=${encodeURIComponent(id)}&v=interior-playable-2`;
}

export function interiorExitUrl(base='./index.html'){
  return `${base}?from=${INTERIOR_RESUME_FROM}`;
}
...
export function loadingOutcome({buildingId,level,error=null}={}){
  ...
  return {
    ...
    message:'Checking the settlement’s shared upgrade level.',
    showRetry:false,
    hideLoading:true
  };
}
