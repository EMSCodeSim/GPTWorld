...
export default async req => {
  ...
  try {
    ...
    // Lock both state rows before evaluating cost. The stockpile is the ONLY funding source.
    const result = await sql`
      WITH locked AS (
        SELECT key,value FROM world_state WHERE key IN ('settlement_stockpile','town_building_levels') ORDER BY key FOR UPDATE
      ), current AS (
        SELECT COALESCE(MAX(value) FILTER (WHERE key='town_building_levels'),'{}'::jsonb) AS levels,
               COALESCE(MAX(value) FILTER (WHERE key='settlement_stockpile'),'{}'::jsonb) AS stock
        FROM locked
      ), cost AS (
        SELECT levels,stock,COALESCE((levels->>${buildingId})::int,1) AS level,
          CASE WHEN COALESCE((levels->>${buildingId})::int,1)=1 THEN ${buildings[buildingId].costs[0].wood}::int ELSE ${buildings[buildingId].costs[1].wood}::int END AS wood_cost,
          CASE WHEN COALESCE((levels->>${buildingId})::int,1)=1 THEN ${buildings[buildingId].costs[0].stone}::int ELSE ${buildings[buildingId].costs[1].stone}::int END AS stone_cost,
          CASE WHEN COALESCE((levels->>${buildingId})::int,1)=1 THEN ${buildings[buildingId].costs[0].herbs}::int ELSE ${buildings[buildingId].costs[1].herbs}::int END AS herbs_cost
        FROM current
      ), spent AS (
        ...
        SELECT ${player[0].id}::bigint,'town_building_upgraded',jsonb_build_object('building',${buildingId}::text,'level',level+1,'wood',wood_cost,'stone',stone_cost,'herbs',herbs_cost) FROM upgraded RETURNING id
      ) SELECT upgraded.value AS levels,spent.value AS stockpile,upgraded.level+1 AS new_level FROM upgraded,spent,logged
    `;
    if (!result.length) return reply({ok:false,error:'insufficient_storehouse_or_max_level'},409);
    return reply({ok:true,levels:result[0].levels,stockpile:resources(result[0].stockpile),buildingId,level:Number(result[0].new_level)});
