...
export function resourceLifecycle(node,{nodeId,resource,max,generation=0,now=new Date(),legacyMature=true}={}){
  ...
  if(plant.stage!=='dead'&&elapsedHours>0){
    // Newly replaced private-world trees should visibly establish in hours, not remain
    // microscopic seedlings for real-world months. Accelerate only the juvenile
    // phase; once mature, normal long-lived biological aging resumes.
    if(resource==='wood'&&Number(plant.age)<AGE_STAGES.mature){
      plant.age=Math.min(AGE_STAGES.mature,Number(plant.age||0)+elapsedHours/6);
    }else{
