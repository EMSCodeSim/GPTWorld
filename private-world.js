...
function renderSurvival(){
  ...
  if(survivalMode==='hunt'){
    ...
    if(animal){
      ...
      const hunt=survivalButton(bow?`🏹 Hunt with ${bowLabel}`:'🏹 Craft a bow to hunt',()=>survivalAction('hunt',{animalId:animal.id,equipment:bow,snapshotAt:survivalData.generatedAt}),!bow||!inRange);
      hunt.classList.add('hunt-primary');actions.push(hunt);
      actions.push(survivalButton('👣 Read tracks',()=>survivalAction('track',{animalId:animal.id,snapshotAt:survivalData.generatedAt})));
      ...
      const trap=(survivalData.equipment||[]).includes('hunting-trap');if(trap)actions.push(survivalButton('🪤 Use hunting trap',()=>survivalAction('hunt',{animalId:animal.id,equipment:'hunting-trap',snapshotAt:survivalData.generatedAt}),!inRange));
    }else{
...
function syncHuntSnapshotAnimals(list=[]){
  for(const animal of list){
    const target=interactables.find(item=>item.type==='animal'&&String(item.livingEntityId)===String(animal.id));
    if(!target?.object)continue;
    target.object.position.set(Number(animal.x||0),0,Number(animal.z||0));
    if(target.object.userData?.target)target.object.userData.target.set(Number(animal.x||0),0,Number(animal.z||0));
    if(target.object.userData)target.object.userData.behavior=animal.behavior||target.object.userData.behavior;
  }
}
...
async function syncPositionNow(){
  ...
  try{
    const response=await fetch(API,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({clientId:activeClientId,action:'save_position',position})});
    const data=await response.json();
    if(!response.ok||!data.ok)throw Error(data.error||'position_sync_failed');
    if(loadedPayload?.session)loadedPayload.session.position={...position};
    setSyncState(cacheAvailable?'synced':'failed');
    return true;
  }catch{
