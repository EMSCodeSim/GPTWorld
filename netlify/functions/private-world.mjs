...
async function ensureResources(sql,world){
  ...
  `;
  const changed=[];
  for(const row of rows){if(row.resource_type!=='wood'&&row.resource_type!=='herbs')continue;const lifecycle=resourceLifecycle({...row.metadata,remaining:Number(row.remaining),generation:Number(row.generation)},{nodeId:row.node_id,resource:row.resource_type,max:Number(row.max_amount),generation:Number(row.generation),now:new Date(),legacyMature:true});if(JSON.stringify(row.metadata?.plant)!==JSON.stringify(lifecycle.plant)||Number(row.remaining)!==Number(lifecycle.remaining)||Number(row.generation)!==Number(lifecycle.generation))changed.push({...row,lifecycle});}
  if(changed.length){
